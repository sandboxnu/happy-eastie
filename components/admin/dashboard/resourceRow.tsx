import { Card, Text, Row, Button, Col, Input, FormElement, Loading } from "@nextui-org/react";
...
const renameResource = async (newName: string, r: WithId<Resource>) => {
  ...
  const requestSettings  : RequestInit=  { method: 'PUT', body: JSON.stringify(requestBody), headers: {'Content-Type': 'application/json'}}
...
export const ResourceRow = (props: ResourceRowProps) => {
  ...
  return (
    <Card onPress={onPress} isHoverable variant="shadow" css={{ borderRadius: 0 }}>
      <Card.Divider />
      <Card.Header id="name" css={{ pb: "$0" }}>
        <Row align="center">
            { editing == EditState.VIEWING ? <Text b size={22} color="$black">{editName}</Text> 
            : <Input value={editName} autoFocus css={{zIndex: 100}} onChange={(e: ChangeEvent<FormElement>) => {setEditName(e.target.value)}}/>}
            <Button onPress={toggleEditing} light auto icon={editing != EditState.LOADING && <img src={editing == EditState.EDITING ? "/checkmark.svg" : "/pencil.svg"}></img>}>
